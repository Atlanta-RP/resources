...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookcaixaeletronico = "https://discord.com/api/webhooks/800987289571688468/cF0SDSZT4lWhPfsqPOdm81uDfv3vi3mcqLbKQpdRH1FDHKlsz_eLbzfXt8hWoNwuWnmV"
		...
		elseif (os.time()-timers) <= 7200 then
			...
			for l,w in pairs(policia) do
				...
				if player then
					async(function()
						...
						TriggerClientEvent('chatMessage',player,"911",{64,64,255},"O roubo começou na ^1Boate Vanilla^0, dirija-se até o local e intercepte os assaltantes.")
					end)
				end
			end
			...
			if player then
				async(function()
					...
					TriggerClientEvent('chatMessage',player,"911",{64,64,255},"O roubo terminou!")
				end)
			end
			return true
		end
	...
	if andamento then
		...
		for l,w in pairs(policia) do
			...
			if player then
				async(function()
					TriggerClientEvent('blip:remover:caixaeletronico',player)
					TriggerClientEvent('chatMessage',player,"911",{64,64,255},"Os assaltante saiu correndo e deixou tudo para trás.")
				end)
			end
		end
	end
...
function func.giveItens()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		random = math.random(31250,37500)
