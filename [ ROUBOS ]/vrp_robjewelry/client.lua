...
func = Tunnel.getInterface("vrp_robjewelry")
func2 = {}
Tunnel.bindInterface("vrp_robjewelry",func2)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if not UsingComputer then
			    ...
			    if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped) and not func.returnJewelry2() then
				    if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_ASSAULTRIFLE") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_SMG") then
					    ...
					    TriggerEvent("Notify","negado","Voce precisa estar com uma <b>AK-103</b> ou uma <b>SMG</b> em suas maos.")
				    end
			    end
			end
		end
	...
	while true do
		...
		if distance <= 20 then
		    for _,v in pairs(locais) do
			    ...
			    if distance2 <= 1.1 and not andamento then
                    uru = 4
			        DrawText3Ds(v.x,v.y,v.z+0.55,"~b~[E] ~w~ROUBAR")
				    if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped) then
				    	if func.returnJewelry() then
						    ...
						    TriggerEvent("Notify","negado","Hackeie as <b>cameras</b> para roubar as <b>joias</b>.")
					    end
				    end
			    end
		    end
...
AddEventHandler("jewelryroubo",function(sec,mod,status,x,y,z,prop1,prop2,id)
	...
	if status then
		...
		if id == 4 or id == 13 or id == 14 or id == 17 then
		    ...
		    Wait(350)
		    TriggerEvent("vrp_sound:source",'vidroquebrado',0.25)
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and ProgramID == 86 then
                    ...
                    TriggerEvent("Notify","sucesso","A proteção das <b>câmeras</b> de <b>segurança</b> foi comprometida, vá até os balcoes e <b>roube</b> as <b>joias</b>, voce ganhara <b>90</b> segundos de vantagem até que a <b>polícia</b> seja acionada.")
                ...
                if Hacking then
                    ...
                    if chanceshack <= 0 then
                        ...
                        Ipfinished = false
                        TriggerEvent('cancelando',false)
                        TriggerEvent("Notify","aviso","Voce falhou em <b>hackear</b> as cameras de segurança, sua conexão foi <b>bloqueada</b> e chamou a <b>polícia</b>.")
                        FreezeEntityPosition(PlayerPedId(),false)
