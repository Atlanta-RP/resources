...
function func.checkReward(tempofinal,index)
	...
	if cops and #cops > 0 then
		bonus = math.random(500,1000) * #cops -- valor random entre 500 e 1000 vezes a quantidade de policiais
	end
	local rank
	...
	if rank == 1 then
		vRP.giveInventoryItem(user_id,"dinheirosujo",reward + bonus + 5000)
		TriggerClientEvent("Notify",source,"sucesso","Parabéns, você recebeu $ <b>"..vRP.format(reward + bonus + 5000).."</b> sujo(s).", 5000)
	else
		vRP.giveInventoryItem(user_id,"dinheirosujo",bonus + 1000)
		TriggerClientEvent("Notify",source,"importante","Você recebeu $ <b>"..vRP.format(bonus + 1000).."</b> sujo(s).", 5000)
	end
