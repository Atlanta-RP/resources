...
vInvetoryServer = Tunnel.getInterface("vrp_inventory")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local db_driver
...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookjoins = "SEU HEBHOOK DE ENTRAR NO SERVER"
...
function vRP.rejoinServer(source)
	...
	if user_id then
		...
		if identity then
			...
			--print(source)
			--print(data)
			-- print(GetAmmoInPedWeapon(source,GetSelectedPedWeapon(source)))
			-- GetSelectedPedWeapon(source)
	...
	if steam then
		if not vRP.isBanned(steam) then
			if vRP.isWhitelisted(steam) then
				...
				deferrals.done("Entre no discord.gg/jWg4bwmXxm e faça sua whitelist, na parte de steamhex coloque: "..steam)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done("Você foi banido da cidade. Em caso de dúvida, ou contestação entre discord.gg/jWg4bwmXxm; Sua steam: "..steam)
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	end
end)
...
AddEventHandler("baseModule:idLoaded",function(source,user_id,model)
	...
	if vRP.rusers[user_id] == nil then
		...
		if model ~= nil then
			...
			vRP.user_tables[user_id].inventorys["3"] = { item = "skate", amount = 1 }
			vRP.user_tables[user_id].inventorys["4"] = { item = "water", amount = 2 }
			vRP.user_tables[user_id].inventorys["5"] = { item = "sandwich", amount = 2 }
			vRP.user_tables[user_id].inventorys["5"] = { item = "dollars", amount = 2000 }
		end
