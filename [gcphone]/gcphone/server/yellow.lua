...
function getUserYellow(phone, name, user)
  MySQL.Async.fetchAll("SELECT * FROM vrp_users WHERE phone = @phone AND name = @name",{ 
    ['@phone'] = phone_number,
    ['@name'] = firstname 
  }, function (data)
    user = data[1]
...
function getIdentity(source)
  ...
  local identifier = vRP.getUserIdentity(user_id)
	local result = MySQL.Sync.fetchAll("SELECT vrp_users FROM vrp_users WHERE vrp_users.id = @identifier",{ ['@identifier'] = identifier })
