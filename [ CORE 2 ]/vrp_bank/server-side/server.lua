...
function cRP.finesPayment(id,price)
	...
	if user_id then
		if vRP.paymentBank(user_id,parseInt(price)) then
			TriggerClientEvent("vrp_bank:Updates12ds",source,"requestFines")
...
function cRP.salaryPayment(id,price)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		TriggerClientEvent("vrp_bank:Updates12ds",source,"requestMySalarys")
...
function cRP.invoicesPayment(id,price,nuser_id)
	...
	if user_id then
		if vRP.paymentBank(user_id,parseInt(price)) then
			...
			TriggerClientEvent("vrp_bank:Updates12ds",source,"requestInvoices")
	...
	if user_id then
		if parseInt(amount) > 0 then
			if vRP.tryGetInventoryItem(user_id,"dollars",parseInt(amount)) then
				...
				TriggerClientEvent("vrp_bank:Updates12ds",source,"requestInicio")
			end
		end
	end
end
