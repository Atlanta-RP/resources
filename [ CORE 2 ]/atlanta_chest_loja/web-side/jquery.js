...
$(document).ready(function(){
	...
	document.onkeyup = data => {
		const key = data.key;
		if (key === "Escape"){
			$.post("http://atlanta_chest_loja/chestClose",JSON.stringify({}));
		} else if (key === "Shift") {
			shiftPressed = false;
		}
	}
...
const updateDrag = () => {
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (tInv === "invLeft") {
				if (origin === "invLeft") {
					$.post("http://atlanta_chest_loja/populateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
					...
					$.post("http://atlanta_chest_loja/takeItem",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt(amount)
					}));
				...
				if ( origin === "invLeft" ) {
					$.post("http://atlanta_chest_loja/storeItem",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						amount: parseInt(amount)
					}));
	...
	$('.populated').droppable({
		...
		drop: function(event,ui){
			...
			if ( tInv === "invLeft" ) {
				if (origin === "invLeft"){
					$.post("http://atlanta_chest_loja/updateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
						amount: parseInt(amount)
					...
					$.post("http://atlanta_chest_loja/sumSlot",JSON.stringify({
						item: itemData.key,
						slot: target,
						amount: parseInt(amount)
					}));
...
const updateChest = () => {
	$.post("http://atlanta_chest_loja/requestChest",JSON.stringify({}),(data) => {
		$(".myInfos").html(`
