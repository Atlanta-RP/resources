...
-- MOC
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterKeyMapping("moc","Abrir a mochila","keyboard","tab")
-- oem_3
-----------------------------------------------------------------------------------------------------------------------------------------
-- DROPITEM
-----------------------------------------------------------------------------------------------------------------------------------------
...
function func.techDistance()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local distance = #(coords - vector3(735.39,-1067.95,22.24))
	if distance <= 10 then
		return true
	end
	return false
...
RegisterCommand("balas",function(source,args)
	local source = vSERVER.getSource()
	local user_id = vSERVER.getUserId()
	local ped = PlayerPedId()
	local data = vSERVER.getUserWeapons(user_id)
	for k, v in pairs(data) do 
		if v.weapon ~= nill then	
			print(v.weapon.."= "..v.ammo)	
			vSERVER.delAmmoLogout(user_id,v.ammo, v.weapon)
		end
	end
	RemoveAllPedWeapons(ped, false)
end)
