...
emP = Tunnel.getInterface("vrp_farmballas")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
local segundos = 0
local CDS = { ['x'] = 120.77, ['y'] = -1937.47, ['z'] = 20.76 }
-----------------------------------------------------------------------------------------------------------------------------------------
-- RESIDENCIAS
-----------------------------------------------------------------------------------------------------------------------------------------
local locs = {
...
Citizen.CreateThread(function()
	local perm = emP.checkPermission()
		while true do
			local uru = 1000
		if not servico and perm == true then
			local ped = PlayerPedId()
			local x,y,z = table.unpack(GetEntityCoords(ped))
			local bowz,cdz = GetGroundZFor_3dCoord(CDS.x,CDS.y,CDS.z)
			local distance = GetDistanceBetweenCoords(CDS.x,CDS.y,cdz,x,y,z,true)
			if distance <= 3 then
				uru = 4
				DrawMarker(3,CDS.x,CDS.y,CDS.z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,232,67,147,100,1,0,0,1)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if servico then
			...
			if distance <= 10 then
				uru = 4
				DrawMarker(3,locs[selecionado].x,locs[selecionado].y,locs[selecionado].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,232,67,147,100,1,0,0,1)
				if distance <= 1.2 then
					...
					if IsControlJustPressed(0,38) and emP.checkItens() and not IsPedInAnyVehicle(ped) then
						...
						if 1 == 1 then
							...
							RemoveBlip(blips)
							-- vRP.tryGetInventoryItem(user_id,"cocaine") = 4
							emP.checkPayment()
