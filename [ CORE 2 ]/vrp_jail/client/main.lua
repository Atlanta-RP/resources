...
function cRP.stopPrison2()
    TriggerEvent("atlanta_hud:toggleHood",false)
    prison = false
    intransport = false
    if DoesBlipExist(blips) then
        RemoveBlip(blips)
...
function cRP.startPrisonLocomove()
    ...
    intransport = true
    Wait(7000)
    TriggerEvent("atlanta_hud:toggleHood",true)
    local ped = PlayerPedId()
    --DoScreenFadeOut(1000)
    disableAction()
    --Wait(1350)
