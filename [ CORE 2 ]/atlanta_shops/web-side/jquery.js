...
const updateDrag = () => {
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (tInv === "invLeft"){
				if (origin === "invLeft"){
					$.post("http://atlanta_shops/populateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
					...
					$.post("http://atlanta_shops/functionShops",JSON.stringify({
						shop: selectShop,
						item: itemData.key,
						slot: target,
						amount: parseInt($(".amount").val())
	...
	$('.populated').droppable({
		...
		drop: function(event,ui){
			...
			if (tInv === "invLeft" ){
				if (origin === "invLeft"){
					$.post("http://atlanta_shops/updateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
						amount: parseInt($(".amount").val())
...
/* --------------------------------------------------- */
const requestShop = () => {
	$.post("http://atlanta_shops/requestShop",JSON.stringify({ shop: selectShop }),(data) => {
		$(".myInfos").html(`
