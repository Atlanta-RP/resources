...
function func.checkPermission()
	...
	return vRP.hasPermission(user_id,"laranjas.permissao") or vRP.hasPermission(user_id,"bahamas.permissao")
end
				...
				if v.componentes then
					for k2,v2 in pairs(v.componentes) do -- VERIFICA SE TEM TODOS OS ITTENS
						if vRP.getInventoryItemAmount(user_id, v2.componente) >= v2.qtd then
							...
							TriggerClientEvent("Notify",source,"negado","Você não possui "..vRP. itemNameList(v2.componente).." suficiente!")
							return false
						end
					end
				...
				if vRP.getInventoryWeight(user_id)+vRP.getItemWeight(v.item)*v.qtd <= vRP.getInventoryMaxWeight(user_id) then
						...
						if not isArma then
							if vRP.tryPayment(user_id,parseInt(v.compra)) then
								...
								TriggerClientEvent("Notify",source,"sucesso","Comprou <b>"..parseInt(v.qtd).."x "..vRP.itemNameList(v.item).."</b> por <b>$"..vRP.format(parseInt(v.compra)).." dólares</b>.")
							else
								TriggerClientEvent("Notify",source,"negado","Dinheiro insuficiente.")
							end
						else
							vRP.giveInventoryItem(user_id,v.item,parseInt(v.qtd))
							TriggerClientEvent("Notify",source,"sucesso","Você recebeu <b>"..parseInt(v.qtd).."x "..vRP.itemNameList(v.item).."</b>")
						end
